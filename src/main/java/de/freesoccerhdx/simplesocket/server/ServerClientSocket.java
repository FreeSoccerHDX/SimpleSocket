...
public class ServerClientSocket extends SocketBase {
	...
	protected static void handleConnection(SimpleSocketServer sss, Socket client) {
		...
		//	System.out.println("a creating obj");
			ServerClientSocket clsock = new ServerClientSocket(sss, client, "<unset>");
		...
		//		System.out.println(" not ");
				System.err.println("Client-Login was not successful! (" + info + ")");
				try{
					client.close();
	...
	protected ServerClientSocket(SimpleSocketServer sss, Socket client, String clientname) throws IOException {
		this.client = client;
		this.sss = sss;
						...
						if (cm.targets.size() > 0) {
							for (String target : cm.targets) {
								if (!target.equals("Server")) {
									...
									if (!sss.handleCustom(ServerClientSocket.this, cm.channelid, cm.targets, cm.message)) {
										System.out.println("---------------");
										System.out.println("---------------");
	...
	public boolean sendNewMessage(String channel, JSONObject jsonmsg, ClientResponse response) {
		...
		try {
			...
			String completemsg = SocketBase.createMessageLengthString(json.toString());
			
			return sendMessage(this.printWriter,completemsg);
		}catch(Exception ex) {
			ex.printStackTrace();
		}
	...
	public boolean sendNewMessage(String channel, String msg, ClientResponse response) {
		...
		try {
			...
			sendMessage(this.printWriter, completemsg);
	...
	//	System.out.println("read next...");
		...
		Pair<SocketBase.SocketInfo, JSONObject> s = super.readNextMessage(this.bufferedReader);
